...
/* ─── Demo interactivo Filtro ─────────────────────────────────────────────── */

const FILTRO_FIELDS = [
  { key: 'fase',    label: 'Fase actual',           options: ['Evaluación', 'Calibración', 'Retroalimentación'] },
  { key: 'avance',  label: 'Avance de formularios',  options: ['Sin avance (0%)', 'Bajo (1%–33%)', 'Medio (34%–66%)', 'Alto (67%–99%)', 'Completado (100%)'] },
  { key: 'dias',    label: 'Días en fase',           options: ['Más de 3 días', 'Más de 7 días', 'Más de 14 días', 'Con alerta (+7 días)'] },
];

const filtroModalOverlayStyle = {
  position: 'fixed', inset: 0, background: 'transparent', zIndex: 400,
};
const filtroModalBaseStyle = {
  position: 'fixed',
  background: '#FFFFFF',
  borderRadius: '16px',
  padding: '16px 0',
  border: '0.5px solid #1E5591',
  boxShadow: '0px 8px 24px rgba(0,0,0,0.12)',
  display: 'flex',
  flexDirection: 'column',
  zIndex: 401,
};
const filtroModalHeaderStyle = {
  padding: '0 16px 12px',
  borderBottom: '1px solid #B6CEE7',
  fontFamily: 'Roboto, sans-serif', fontSize: '16px', fontWeight: 600,
  color: '#1E5591', margin: 0,
};
const filtroModalOptionStyle = (active) => ({
  display: 'flex', alignItems: 'center', justifyContent: 'space-between',
  width: '100%', padding: '12px 16px', border: 'none',
  borderBottom: '1px solid #B6CEE7', background: 'none', cursor: 'pointer',
  fontFamily: 'Roboto, sans-serif', fontSize: '14px',
  color: '#5780AD', fontWeight: active ? 600 : 400, textAlign: 'left',
});

function FiltroDemo() {
  const [open, setOpen] = useState(false);
  const [filterValues, setFilterValues] = useState({});
  const [activeField, setActiveField] = useState(null); /* { field, anchorRect } */

  function handleFieldClick(key, field, e) {
    const rect = e.currentTarget.getBoundingClientRect();
    setActiveField({ field, anchorRect: rect });
  }
  ...
  const fieldsWithValues = FILTRO_FIELDS.map((f) => ({ ...f, value: filterValues[f.key] ?? null }));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      ...
      <p style={{ fontSize: '12px', color: '#9ca3af', margin: 0 }}>
        Estado: <strong style={{ color: open ? c.importante.$value : c.dash.$value }}>
          {open ? 'Activo (panel desplegado)' : 'Default'}
        </strong>
      </p>

      {activeField && (
        <div style={filtroModalOverlayStyle} onPointerDown={() => setActiveField(null)}>
          <div
            style={{
              ...filtroModalBaseStyle,
              top: activeField.anchorRect.top,
              left: activeField.anchorRect.left,
              width: activeField.anchorRect.width,
            }}
            onPointerDown={(e) => e.stopPropagation()}
          >
            <p style={filtroModalHeaderStyle}>{activeField.field.label}</p>
            <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
              {activeField.field.options.map((opt, i) => {
                const isLast = i === activeField.field.options.length - 1;
                const selected = filterValues[activeField.field.key] === opt;
                return (
                  <li key={opt}>
                    <button
                      style={{ ...filtroModalOptionStyle(selected), ...(isLast ? { borderBottom: 'none' } : {}) }}
                      onClick={() => handleSelect(activeField.field.key, opt)}
                    >
                      <span>{opt}</span>
                      {selected && (
            ...
            {filterValues[activeField.field.key] && (
              <button
                style={{ background: 'none', border: 'none', borderTop: '1px solid #B6CEE7', padding: '12px 16px', cursor: 'pointer', color: '#999', fontFamily: 'Roboto, sans-serif', fontSize: '13px', textAlign: 'left' }}
                onClick={() => handleSelect(activeField.field.key, null)}
              >
                Limpiar filtro
              </button>
            )}
    ...
    <div style={pageStyle}>
      ...
      <main style={mainStyle}>
        ...
        <section id="botones" style={sectionStyle}>
          ...
          <div style={rowStyle}>
            <ShowcaseItem label="disabled (primary)">
              <Button disabled>Deshabilitado</Button>
            </ShowcaseItem>
            <ShowcaseItem label="disabled (secondary outline)">
              <Button variant="secondary" disabled>Fase anterior</Button>
            </ShowcaseItem>
            ...
            <ShowcaseItem label="icon derecha (secondary)">
              <Button
                variant="secondary"
                iconPosition="right"
                icon={
        ...
        {/* ══════════════════════════════════════════════
            22. FILTRO
        ══════════════════════════════════════════════ */}
        <section id="filtro" style={sectionStyle}>
          <div style={sectionHeadStyle}>
            <h2 style={sectionTitleStyle}>Filtro</h2>
            <p style={sectionDescStyle}>
              Filtro — pill interactivo de filtro. 3 estados: Default · Activo · Disable.
              Íconos: Filter + ChevronDown (src/assets/svg/). Chevron rota 180° en estado activo.
              Fuente: Figma W1ux236a3TuleZ3d2tjAZ6 · nodo 3375:3289.
            </p>
          </div>

          <GroupLabel>Default — pill compacto</GroupLabel>
          <div style={{ ...rowStyle, marginBottom: '28px', alignItems: 'center' }}>
            <ShowcaseItem label="Default">
              <Filtro />
            </ShowcaseItem>
            <ShowcaseItem label="Disable">
              <Filtro disabled />
            </ShowcaseItem>
          </div>
          ...
          <GroupLabel>Activo — panel expandido con ítems</GroupLabel>
          <div style={{ position: 'relative', minHeight: '340px', marginBottom: '28px' }}>
            <ShowcaseItem label="Activo (expanded=true)">
              <Filtro
                expanded
                fields={FILTRO_FIELDS}
                onToggle={() => {}}
              />
            </ShowcaseItem>
          </div>
          ...
          <GroupLabel>Demo interactivo — clic en "Filtros" para abrir/cerrar el panel</GroupLabel>
          <FiltroDemo />
        </section>
        ...
        {/* ══════════════════════════════════════════════
            23. HELPER
        ══════════════════════════════════════════════ */}
        <section id="helper" style={sectionStyle}>
          <div style={sectionHeadStyle}>
            <h2 style={sectionTitleStyle}>Helper</h2>
            <p style={sectionDescStyle}>
              Popover contextual para explicar términos, componentes o estados en contexto.
              Fondo primario · Título en auxiliar · Texto en blanco · CTA en exito.
            </p>
          </div>

          <GroupLabel>Sin título — solo texto y CTA</GroupLabel>
          <div style={{ ...rowStyle, marginBottom: '28px' }}>
            <ShowcaseItem label="default">
              <Helper
                supportingText="Los colaboradores avanzan de fase automáticamente al completar el 100% de sus tareas."
                cta="Ver más"
              />
            </ShowcaseItem>
          </div>
          ...
          <GroupLabel>Con título</GroupLabel>
          <div style={{ ...rowStyle, marginBottom: '28px' }}>
            <ShowcaseItem label="con título + CTA">
              <Helper
                title="Avance diferenciado"
                supportingText="Cada colaborador avanza de fase de forma independiente al completar sus formularios."
                cta="Entendido"
              />
            </ShowcaseItem>
            <ShowcaseItem label="solo texto, sin CTA">
              <Helper
                title="Fase de calibración"
                supportingText="En esta fase los líderes revisan y ajustan las calificaciones de forma comparativa."
              />
            </ShowcaseItem>
          </div>
          ...
          <GroupLabel>Posiciones del pointer</GroupLabel>
          <div style={{ ...rowStyle, flexWrap: 'wrap', gap: '40px', paddingTop: '16px', paddingBottom: '16px' }}>
            {(['top', 'bottom', 'left', 'right']).map((pos) => (
              <ShowcaseItem key={pos} label={`pointer: ${pos}`}>
                <div style={{ padding: '12px' }}>
                  <Helper
                    title="Información"
                    supportingText="Este elemento requiere tu atención antes de continuar."
                    cta="OK"
                    pointer={pos}
                  />
                </div>
        ...
        {/* ══════════════════════════════════════════════
            24. ÍCONOS
        ══════════════════════════════════════════════ */}
        <section id="iconos" style={sectionStyle}>
          <div style={sectionHeadStyle}>
            <h2 style={sectionTitleStyle}>Íconos</h2>
            <p style={sectionDescStyle}>
              Íconos SVG inline — props <code>size</code> (default 20) y <code>color</code> (default currentColor).
              Ubicación: <code>src/assets/icons/</code>
            </p>
          </div>

          <GroupLabel>Catálogo (tamaño 20px)</GroupLabel>
          <div style={{ ...rowStyle, flexWrap: 'wrap', marginBottom: '28px' }}>
            {[
              { label: 'EditIcon',       Icon: EditIcon       },
              { label: 'EyeIcon',        Icon: EyeIcon        },
              { label: 'TrashIcon',      Icon: TrashIcon      },
              { label: 'ArrowRightIcon', Icon: ArrowRightIcon },
              { label: 'CheckIcon',      Icon: CheckIcon      },
              { label: 'CheckFillIcon',  Icon: CheckFillIcon  },
              { label: 'XIcon',          Icon: XIcon          },
            ].map(({ label, Icon }) => (
              <ShowcaseItem key={label} label={label}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 48, height: 48, background: '#fff', borderRadius: 8, border: `1px solid ${c.auxiliar.$value}` }}>
                  <Icon size={20} color={c.primario.$value} />
                </div>
              </ShowcaseItem>
            ))}
          </div>
          ...
          <GroupLabel>Tamaños (EditIcon)</GroupLabel>
          <div style={{ ...rowStyle, alignItems: 'center' }}>
            {[16, 20, 24, 32].map((s) => (
              <ShowcaseItem key={s} label={`${s}px`}>
                <EditIcon size={s} color={c.primario.$value} />
              </ShowcaseItem>
            ))}
          </div>
          ...
          <GroupLabel>Colores semánticos (CheckFillIcon)</GroupLabel>
          <div style={rowStyle}>
            {[
              { label: 'exito',      color: c.exito.$value      },
              { label: 'alerta',     color: c.alerta.$value     },
              { label: 'error',      color: c.error.$value      },
              { label: 'importante', color: c.importante.$value },
              { label: 'primario',   color: c.primario.$value   },
            ].map(({ label, color }) => (
              <ShowcaseItem key={label} label={label}>
                <CheckFillIcon size={24} color={color} />
              </ShowcaseItem>
            ))}
          </div>
        </section>
